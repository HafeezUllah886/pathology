                        ...
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="patient_name" required id="patient_name"
                                        class="form-control" value="{{ $receipt->patient_name }}">
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="patient_gender" id="patient_gender" class="form-control">
                                        <option value="Male" {{ $receipt->patient_gender == 'Male' ? 'selected' : '' }}>
                                            Male</option>
                                        <option value="Female" {{ $receipt->patient_gender == 'Female' ? 'selected' : '' }}>
                                            Female</option>
                                        <option value="Other" {{ $receipt->patient_gender == 'Other' ? 'selected' : '' }}>
                                            Other</option>
                                    </select>
                                </div>
                            </div>
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <div class="input-group">
                                        <input type="text" name="patient_contact" id="patient_contact"
                                            placeholder="3456789012" class="form-control"
                                            value="{{ $receipt->patient_contact }}">
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <table class="table">
                                        ...
                                        <tbody id="tests_list">
                                            @foreach ($receipt->receipt_tests as $test)
                                                ...
                                                @endphp
                                                <tr id="row_{{ $id }}">
                                                    <td class="no-padding"><span
                                                            class="text-info font-size-18">{{ $test->test->name }}</span><br>{{ $test->test->parameters->pluck('title')->implode(', ') }}
                                                    </td>
                                                    <td class="no-padding">{{ $test->reporting_time }}</td>
                                                    <td class="no-padding"><input type="number" name="rate[]"
                                                            oninput="updateChanges()" min="0" required step="any"
                                                            value="{{ $test->price }}"
                                        ...
                                        <tfoot>
                                            ...
                                            <tr>
                                                ...
                                                <th class="no-padding"><input type="number" name="discount"
                                                        value="{{ $receipt->discount }}" id="discount"
                                                        oninput="updateChanges()"
                                                        class="form-control text-center m-0 no-padding"></th>
                                                <th></th>
                                            </tr>
                                            <tr>
                                                <th colspan="2" class="text-end no-padding">Net Total</th>
                                                <th id="net_total" class="no-padding text-end">{{ $receipt->net_amount }}
                                                </th>
                                                <th></th>
                                            </tr>
                                        </tfoot>
                                ...
                                <div class="col-md-3 mt-2">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="refered_by" id="refered_by"
                                            value="{{ $receipt->refered_by }}" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-3 mt-2">
                                    <div class="form-group">
                                        <label for="name">Entery Time</label>
                                        <input type="datetime-local" name="entery_time"
                                            value="{{ $receipt->entery_time }}" id="entery_time" class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-3 mt-2">
                                    <div class="form-group">
                                        <label for="name">Reporting Time</label>
                                        <input type="datetime-local" name="reporting_time"
                                            value="{{ $receipt->reporting_time }}" id="reporting_time"
                                            class="form-control">
                                    </div>
                                </div>
                                <div class="col-md-3 mt-2">
                                        ...
                                        <select name="paid_in" id="paid_in" class="form-control">
                                            @foreach ($accounts as $account)
                                                <option value="{{ $account->id }}"
                                                    {{ $account->id == $receipt->paid_in ? 'selected' : '' }}>
            ...
            $.ajax({
                ...
                success: function(test) {
                    ...
                    if (found.length > 0) {} else {
                        ...
                        html += '<td class="no-padding"><span class="text-info font-size-18">' + test.name +
                            '</span><br>' + test.parameters + '</td>';
                        html += '<td class="no-padding">' + test.report_time + '</td>';
                        html +=
                            '<td class="no-padding"><input type="number" name="rate[]" oninput="updateChanges()" min="0" required step="any" value="' +
                            test.rate + '" class="form-control text-center no-padding" id="rate_' + id +
                            '"></td>';
                        html +=
